...
async def ai_chat(
    ...
    is_patient_only = user_roles == {RoleEnum.PATIENT.value}

    if is_patient_only:
        raise HTTPException(403, "AI-асистент доступний лише лікарям")
    ...
    # Перевірка доступу: учасник або адмін або терапевт пацієнта
    if not is_admin:
